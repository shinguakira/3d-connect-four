...
describe("GameManager", () => {
  ...
  describe("makeMove", () => {
    it("rejects moves before the game is started", () => {
      ...
      const result = mgr.makeMove(room.id, "h", { playerId: "h", x: 0, z: 0, timestamp: new Date() })
    ...
    it("rejects moves from the wrong player", () => {
      ...
      const result = mgr.makeMove(room.id, "g", { playerId: "g", x: 0, z: 0, timestamp: new Date() })
      expect(result).toBeNull()
    })
    ...
    it("places a piece, alternates turn, and updates lastActivity", () => {
      ...
      const after = mgr.makeMove(room.id, "h", { playerId: "h", x: 1, z: 2, timestamp: new Date() })
    ...
    it("stacks pieces by gravity in the same column", () => {
      ...
      const final = mgr.makeMove(room.id, "h", { playerId: "h", x: 0, z: 0, timestamp: new Date() })
    ...
    it("declares a winner and freezes the game when 4 in a row form", () => {
      ...
      const denied = mgr.makeMove(room.id, "g", { playerId: "g", x: 3, z: 3, timestamp: new Date() })
      expect(denied).toBeNull()
    })
    ...
    it("rejects a move into a full column", () => {
      ...
      const denied = mgr.makeMove(room.id, "h", { playerId: "h", x: 0, z: 0, timestamp: new Date() })
      expect(denied).toBeNull()
    })
  })
