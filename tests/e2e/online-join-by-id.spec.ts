...
test.describe("Online: join by room id", () => {
  ...
  test("guest joining the host's room id puts both clients in the same room", async ({ browser }) => {
  ...
  test("rejects joining a full room", async ({ browser, request }) => {
    ...
    try {
      ...
      await pageGuest2.getByRole("button", { name: /オンライン/ }).first().click()
      ...
      await expect(pageGuest2.getByText(/ルーム参加に失敗しました|ルームが見つからない/)).toBeVisible({
        timeout: 10_000,
      })
    } finally {
      await ctxHost.close()
